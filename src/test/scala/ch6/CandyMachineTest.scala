...
class simulate_candy_machine extends munit.FunSuite:
  ...
  test("should dispense 0 candy and 0 coin when initial state has 0 candy and 0 coin"):
  ...
  test("should dispense 1 candy and 0 coin when initial state has 1 candy and 0 coin and no coin is inserted"):
    val machine = Machine(true, 1, 0)
    val finalState = machine.simulateMachine(List()).run(machine)
    assertEquals(finalState._1, (0, 1))

  test("should unlock the machine when initial state is locked with 1 candy and 0 coin, and a coin is inserted"):
    val machine = Machine(true, 1, 0)
    val finalState = machine.simulateMachine(List(Input.Coin)).run(machine)
    assertEquals(finalState, ((0, 1), Machine(false, 1, 0)))

  test("should dispense a candy when the machine is unlocked with 1 candy and 0 coin, and the knob is turned"):
  ...
  test("should lock the machine when a candy is dispensed"):
    val machine = Machine(false, 1, 0)
    val finalState = machine.simulateMachine(List(Input.Turn)).run(machine)
